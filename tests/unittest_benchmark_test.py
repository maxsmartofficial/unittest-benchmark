...
class BenchmarkTest(unittest.TestCase):
    ...
    def test_strict_p_value_fails(self):
        with self.assertRaises(self.mock_test_case.failureException):
            self.mock_test_case.assertIsFaster(self.mostly_fast_callable, fast_function, p_value=0.0001)

    def test_lenient_p_value_succeeds(self):
        try:
            self.mock_test_case.assertIsFaster(self.mostly_fast_callable, slow_function, p_value=0.1)
        except self.mock_test_case.failureException:
            self.fail("The faster function was not identified despite the lenient p-value")

    def test_more_samples_succeeds(self):
        try:
            self.mock_test_case.assertIsFaster(self.mostly_fast_callable, slow_function, p_value=0.0001, samples=100)
        except self.mock_test_case.failureException:
